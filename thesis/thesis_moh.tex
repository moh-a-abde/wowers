...
\wptodo{M2 · Ch5.1.6 (~700 w)}{Feeds: journal 2026-07-06 PM\#5 and
2026-07-07 verification notes, \texttt{npm run build} output. Beats: build
performance (vite version, build time, bundle sizes --- e.g.\ maplibre and
index chunk sizes); headless + Chrome QA results (console errors, network
requests, interaction checks); the mandatory discrepancy paragraph for the
bundle-size / code-splitting deferral (state the gap, name the mechanism,
say if acceptable for current scope, name the fix). Separate what was
\textbf{measured} (build times, bundle sizes, QA pass/fail) from what is
\textbf{deferred} (code-splitting, map clustering, live backend). Numbers
must be reproduced live this session, not copied from a report --- rerun
\texttt{npm run build} and the QA checks rather than trusting old figures.}
